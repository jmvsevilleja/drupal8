...
namespace Drupal\drupal8_profile_content;
...
class InstallHelper implements ContainerInjectionInterface {
  ...
  public function importContent() {
    $this->getModulePath()
      ...
      ->importContentFromFile('taxonomy_term', 'category')
  ...
  protected function getModulePath() {
    $this->module_path = $this->moduleHandler->getModule('drupal8_profile_content')->getPath();
    return $this;
  }
...
/*    if (!empty($data['body'])) {
    ...
    // Set body field.
    if (!empty($data['body'])) {
      $body_path = $this->module_path . '/default_content/languages/' . $langcode . '/page_body/' . $data['body'];
    ...
    if (!empty($data['description'])) {
      $page_description_path = $this->module_path . '/default_content/languages/' . $langcode . '/page_description/' . $data['description'];
      $page_description = file_get_contents($page_description_path);
      if ($page_description !== FALSE) {
        $values['field_description'] = [['value' => $page_description, 'format' => 'basic_html']];
      }
    }
    return $values;
  }
  ...
  protected function processArticle(array $data, $langcode) {
    ...
    // Set field_category if exists.
    if (!empty($data['category'])) {
      $values['field_category'] = [];
      $tags = array_filter(explode(',', $data['category']));
      foreach ($tags as $tag_id) {
        if ($tid = $this->getTermId('category', $tag_id)) {
  ...
  public function deleteImportedContent() {
    $uuids = $this->state->get('drupal8_profile_content_uuids', []);
  ...
  protected function storeCreatedContentUuids(array $uuids) {
    $uuids = $this->state->get('drupal8_profile_content_uuids', []) + $uuids;
    $this->state->set('drupal8_profile_content_uuids', $uuids);
  }
